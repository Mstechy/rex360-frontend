    ...
    <div className="bg-white min-h-screen pb-20 selection:bg-green-100">
      <div className="bg-slate-50 border-b border-slate-200">
        <div className="container mx-auto px-6 py-4 flex items-center gap-2 text-xs font-bold text-slate-400 uppercase tracking-widest">
          <Link to="/" className="hover:text-green-700">Home</Link> <ChevronRight size={12} /> <Link to="/blog" className="hover:text-green-700">News</Link> <ChevronRight size={12} /> <span className="text-slate-900 truncate">{post.title}</span>
        </div>
      </div>
          ...
          <div className="lg:w-2/3">
            <header className="mb-10">
              ...
              <div className="flex flex-wrap items-center gap-4 text-slate-400 text-[10px] font-black uppercase tracking-[0.2em] border-y border-slate-100 py-4">
                 <div className="flex items-center gap-2"><Calendar size={14} className="text-green-600"/> {new Date(post.created_at).toLocaleDateString()}</div> | <div>Management</div>
              </div>
            </header>
            <div className="prose prose-lg text-slate-700 leading-relaxed">
              ...
              <div className="whitespace-pre-wrap">{post.content || "Contact REX360 for full clarification on this notice."}</div>
              <div className="mt-20 pt-10 border-t-2 border-slate-100 bg-slate-50 p-8 rounded-2xl">
                 ...
                 <p className="text-slate-400 text-xs mt-2 uppercase">REX360 SOLUTIONS LTD • RC 142280</p>
              </div>
            </div>
          </div>
          <aside className="lg:w-1/3">
            <div className="sticky top-24 bg-slate-900 rounded-3xl p-8 text-white shadow-2xl">
              ...
              <Link to="/services" className="block text-center bg-green-600 text-white font-bold py-3 rounded-xl">Consult Now</Link>
            </div>
          </aside>
        </div>
      </div>
